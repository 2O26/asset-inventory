...
// List of 1000 common ports to scan
...
func ping(addr string) (bool, error) {
	start := time.Now()
	fmt.Println("Starting ping at", start)
	...
	for i := 0; i < 3; i++ {
		...
		if err != nil {
			if neterr, ok := err.(net.Error); ok && neterr.Timeout() {
				...
				return false, fmt.Errorf("failed to read ICMP response: %w", err)
			}
		}

		rm, err := icmp.ParseMessage(ipv4.ICMPTypeEchoReply.Protocol(), rb[:n])
		if err != nil {
			return false, fmt.Errorf("failed to parse ICMP response: %w", err)
		}
		...
		attemptEnd := time.Now()
		fmt.Println("Attempt", i+1, "ended at", attemptEnd, "duration:", attemptEnd.Sub(attemptStart))
	}

	end := time.Now()
	fmt.Println("Ping ended at", end, "duration:", end.Sub(start))
